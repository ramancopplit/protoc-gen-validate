...
const repTpl = `
	...
	{{ if $r.GetMinItems }}
		{{ if eq $r.GetMinItems $r.GetMaxItems }}
			if len({{ accessor . }}) != {{ $r.GetMinItems }} {
				err := {{ errWithCode . $r.GetErrorCode  "value must contain exactly " $r.GetMinItems " item(s)" }}
				if !all { return err }
				errors = append(errors, err)
			}
		{{ else if $r.MaxItems }}
			if l := len({{ accessor . }}); l < {{ $r.GetMinItems }} || l > {{ $r.GetMaxItems }} {
				err := {{ errWithCode . $r.GetErrorCode  "value must contain between " $r.GetMinItems " and " $r.GetMaxItems " items, inclusive" }}
				if !all { return err }
				errors = append(errors, err)
			}
		{{ else }}
			if len({{ accessor . }}) < {{ $r.GetMinItems }} {
				err := {{ errWithCode . $r.GetErrorCode  "value must contain at least " $r.GetMinItems " item(s)" }}
				if !all { return err }
				errors = append(errors, err)
			}
		{{ end }}
	{{ else if $r.MaxItems }}
		if len({{ accessor . }}) > {{ $r.GetMaxItems }} {
			err := {{ errWithCode . $r.GetErrorCode  "value must contain no more than " $r.GetMaxItems " item(s)" }}
			if !all { return err }
			errors = append(errors, err)
		}
	{{ end }}
		...
		for idx, item := range {{ accessor . }} {
			...
			{{ if $r.GetUnique }}
				if _, exists := {{ lookup $f "Unique" }}[{{ if isBytes $f.Type.Element }}string(item){{ else }}item{{ end }}]; exists {
					err := {{ errIdxWithCode . $r.GetErrorCode  "idx" "repeated value must contain unique items" }}
					if !all { return err }
					errors = append(errors, err)
				} else {
